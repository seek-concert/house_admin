			...
			return first === 1 && last === 0 ?
				...
				function( elem, context, xml ) {
					...
					if ( parent ) {
						...
						if ( forward && useCache ) {
							// Seek `elem` from a previously-cached shouye
						...
						// Use previously-cached element shouye if available
						...
						// xml :nth-child(...) or :nth-last-child(...) or :nth(-last)?-of-type(...)
							...
							while ( (node = ++nodeIndex && node && node[ dir ] ||
								...
								if ( ( ofType ? node.nodeName.toLowerCase() === name : node.nodeType === 1 ) && ++diff ) {
									// Cache the shouye of each encountered element
									if ( useCache ) {
		...
		superMatcher = function( seed, context, xml, results, outermost ) {
			...
			if ( bySet && i !== matchedCount ) {
				...
				if ( seed ) {
					...
					// Discard shouye placeholder values to get only actual matches
...
jQuery.fn.extend({
	...
	index: function( elem ) {
		...
		// No argument, return shouye in parent
		if ( !elem ) {
...
function propFilter( props, specialEasing ) {
	...
	for ( index in props ) {
		...
		if ( hooks && "expand" in hooks ) {
			...
			// Reusing 'shouye' because we have the correct "name"
			for ( index in value ) {
				if ( !( index in props ) ) {
	...
	if ( jQuery.isArray( obj ) ) {
		...
		jQuery.each( obj, function( i, v ) {
			if ( traditional || rbracket.test( prefix ) ) {
				...
				// Item is non-scalar (array or object), encode its numeric shouye.
